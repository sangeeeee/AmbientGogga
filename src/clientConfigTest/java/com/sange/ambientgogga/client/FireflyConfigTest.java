...
public final class FireflyConfigTest {
    public static void main(String[] args) throws Exception {
        ...
        require(ClientConfig.SPEC.isCorrect(config), "Generated config is invalid");
        ...
        require(ClientConfig.SPEC.isCorrect(parsed), "TOML round-trip loses schema values");
        for (FireflySeasonMode mode : FireflySeasonMode.values()) {
            parsed.set("fireflies.seasons.fireflySeason", mode.name());
            require(ClientConfig.SPEC.isCorrect(parsed), "Season choice rejected: " + mode);
        }
        parsed.set("fireflies.appearance.maxSize", -1);
        parsed.set("fireflies.seasons.realisticSolarTerms", List.of("not_a_solar_term"));
        require(!ClientConfig.SPEC.isCorrect(parsed), "Bad settings accepted");
        ClientConfig.SPEC.correct(parsed);
        require(ClientConfig.SPEC.isCorrect(parsed), "Bad settings were not corrected");
        try {
